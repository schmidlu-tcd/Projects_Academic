...
int findElement(char *name)
{
    ...
    while(i < ARRAY_SIZE)
    {
        if(elements[i].head != NULL && !strcmp(elements[i].head->Surname, name)) //if head is not NULL 
        {
            //if name is found, return location of linked list
            return i;
        }
        i++;
...
void newElement(Node *temp)
{
    ...
    while(elements[position].head != NULL && strcmp(elements[position].head->Surname, temp->Surname))
    {
        collisions++;
        position++;
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        int pos = findElement(input);

        if(pos != -1) //prints names
        {
